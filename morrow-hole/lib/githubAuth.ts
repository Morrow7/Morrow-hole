...
export async function requireGithubUser(request: NextRequest) {
  ...
  if (!res.ok) {
    ...
    return { ok: false as const, response: NextResponse.json({ message: "github_request_failed", status: res.status }, { status: 502 }) }
  }
  ...
  if (!login) {
    return { ok: false as const, response: NextResponse.json({ message: "invalid_user" }, { status: 502 }) }
  }

  return { ok: true as const, user: { login, name, avatarUrl } satisfies GithubUser }
}
